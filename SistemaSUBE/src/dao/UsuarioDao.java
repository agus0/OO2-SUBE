...
public class UsuarioDao {
	...
	public int agregar(Usuario objeto) {int id = 0;
		try {
			iniciaOperacion();
	...
	public Usuario traerI( long idUsuario) throws HibernateException {
		Usuario objeto = null ;
		try {
			iniciaOperacion();
	...
	public Usuario traer( int dni) throws HibernateException {
		Usuario objeto = null ;
		try {
			iniciaOperacion();
	...
	public List<Usuario> traer() throws HibernateException {
		...
		try {
			iniciaOperacion();
			lista= session .createQuery( "from Usuario c order by c.apellido asc c.nombre asc" ).list();
		} finally {
			session .close();
		}
		return lista;
	...
	public Usuario traerUsuarioYBeneficio(long idUsuario){
		Usuario objeto = null;
		try{
			iniciaOperacion();
			String hql = "from Usuario c inner join fetch c.beneficio where c.idUsuario ="+idUsuario;
